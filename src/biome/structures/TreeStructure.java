    ...
    private void buildOakTree(IBlockHolder holder, int x, int y_, int z){
        ...
        for(int i = -rad; i <= rad; i++) {
            for(int j = -rad; j <= rad; j++) {
                for(int a = 0; a <= 1; a++) {
                    short b = holder.getBlock(x + i, y+height-2+a, z + j);
                    if (b == Block.AIR)
                        if(!(a==0 && i == -rad && j == -rad) && !(a==1 && i == rad && j == rad))
        ...
        for(int i = -1; i <= 1; i++) {
            for(int j = -1; j <= 1; j++) {
                for(int a = 0; a <= 2; a++) {
                    short b = holder.getBlock(x + i, y + height-1+a, z + j);
                    if (b == Block.AIR)
                        if (!(a == 0 && i == -1 && j == -1) && !(a == 1 && i == 1 && j == 1 && i == -1 && j == -1))
